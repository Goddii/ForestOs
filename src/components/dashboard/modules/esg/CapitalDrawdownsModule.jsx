...
const USD = (n) => `$${(n / 1_000_000).toFixed(2)}M`
...
const COLUMNS = [
  ...
  { key: 'amountUsd', label: 'Amount', align: 'right', mono: true, sortAccessor: (r) => r.amountUsd },
  { key: 'status', label: '', align: 'right', sortAccessor: (r) => r.status },
]

export default function CapitalDrawdownsModule() {
  const { drawdowns, cumulativeDrawnUsdM } = ESG
  const drawn = drawdowns.filter((d) => d.status === 'drawn').reduce((s, d) => s + d.amountUsd, 0)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatTile label="Drawn to date" value={USD(drawn)} unit={`${drawdowns.filter((d) => d.status === 'drawn').length} tranches`} tone="positive" />
        <StatTile label="Scheduled" value={USD(scheduled)} unit="next 2 quarters" />
        <StatTile label="Next call" value={next ? next.date : '—'} unit={next ? USD(next.amountUsd) : ''} tone="warn" />
        <StatTile label="Avg tranche" value={USD(drawn / drawdowns.filter((d) => d.status === 'drawn').length)} />
      </div>

      <Panel title="Cumulative capital drawn" lede="Running total of released tranches, USD millions.">
        <Sparkline values={cumulativeDrawnUsdM} />
      ...
      <Panel title="Tranche ledger" lede="Every capital call and its milestone.">
        <DataTable
          ...
          renderCell={(key, row) => {
            if (key === 'amountUsd') return USD(row.amountUsd)
