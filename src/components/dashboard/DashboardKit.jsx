...
// #059669, amber-700 #7c5322, ink-muted).
const TREND_STROKE = { ink: '#4f5c52', emerald: '#059669', amber: '#7c5322' }
const TREND_FILL = {
  ink: 'rgba(79,92,82,0.12)',
 ...
 * never a lone figure in an empty box.
 */
export function StatTile({ label, value, unit, delta, trend, share, tone = 'default' }) {
  const valueColor = tone === 'positive' ? 'text-emerald-700' : 'text-ink'
  const warn = tone === 'warn'
  const markTone = warn ? 'bg-amber-500' : 'bg-emerald-600'
  return (
    <div
      className={
        'flex flex-col rounded-lg bg-card p-4 shadow-card ' +
        (warn ? 'border border-amber-700/25' : CARD_BORDER)
      }
    >
      <p className="flex items-center gap-1.5 font-mono text-[11px] font-medium uppercase tracking-[0.12em] text-ink-faint">
        {warn && <span className="h-1.5 w-1.5 rounded-full bg-amber-500" aria-hidden="true" />}
        {label}
      </p>
      <div className="mt-2 flex items-end justify-between gap-2">
        <p className={'font-display text-[2rem] leading-none tabular-nums ' + valueColor}>{value}</p>
        {trend ? (
          <MiniSparkline values={trend} tone={warn ? 'amber' : 'emerald'} />
        ) : typeof share === 'number' ? (
          <span
...
const STATUS_TONE = {
  ...
  approved: 'positive', active: 'positive', passed: 'positive', verified: 'positive',
  'outcome recorded': 'positive',
  watch: 'warn', flagged: 'warn', full: 'warn', overdue: 'warn', open: 'warn',
  rejected: 'warn', 'at risk': 'warn', escalated: 'warn',
}
