...
// A `fromBatchRecord` is intentionally omitted — the dashboard reads batch
// provenance through `redactBatchRecord()` in `batchChain.js`, which is already
// the single shared source.

// ── Conservation passport ──────────────────────────────────────────────────

/** The buyer's computed passport aggregate → ConservationPassport contract. */
export function toConservationPassport(p, { passportId, brand, status = 'active' }) {
  return {
    passportId,
    brand,
    bufferZones: p.bufferZones ?? [],
    volumeSourcedKg: p.volumeKg ?? 0,
    conservationActivity: [
      p.conservation?.covenantHa ? `${p.conservation.covenantHa.toLocaleString()} ha under buffer covenant` : null,
      p.conservation?.seedlingsPlanted ? `${p.conservation.seedlingsPlanted.toLocaleString()} seedlings planted` : null,
    ]
      .filter(Boolean)
      .join(', '),
    farmersRepresented: p.farmersRepresented ?? 0,
    bufferHectaresAttributed: p.bufferHa ?? 0,
    verificationRecords: (p.verifications ?? []).map((v) => v.reference).filter(Boolean),
    status,
  }
}
